...
fn run(h: usize, w: usize, s: Vec<&str>) -> Vec<(isize, isize)> {
    ...
    for i in 0..h {
        for j in 0..w {
            for k in 0..4 {
                let mut current = (i as isize, j as isize);
                ...
                for _ in 0..5 {
                    ...
                    // 端っこを超えたらやめる
                    if current.0 + dy[k] < 0 || current.0 + dy[k] >= h as isize {
                        continue;
                    }

                    // 端っこを超えたらやめる
                    if current.1 + dx[k] < 0 || current.1 + dx[k] >= w as isize {
                        continue;
                    }

                    current.0 = (current.0 + dy[k] + h as isize) % h as isize;
                    current.1 = (current.1 + dx[k] + w as isize) % w as isize;
                }
                ...
                if str == "snuke" {
                    let ans: Vec<(isize, isize)> = pos.iter().map(|(i, j)| (i+1, j+1)).collect();

                    return ans;
                }

                if str_rev == "snuke" {
                    let ans: Vec<(isize, isize)> = pos.iter().rev().map(|(i, j)| (i+1, j+1)).collect();
...
mod tests {
    ...
    struct TestCase(usize, usize, Vec<&'static str>, Vec<(isize, isize)>);
